...
let
  prl-tools = config.modules.services.parallels.package;
in

{
  ...
  config = mkIf config.modules.services.parallels.enable {
    services.xserver = {
      drivers = singleton
    ...
    systemd.services.prlfsmountd = mkIf config.modules.services.parallels.autoMountShares {
